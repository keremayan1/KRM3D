...
namespace Core.Persistance.Dynamic
{
    public static class IQueryableDynamicFilterExtensions
    {
        ...
        private static IQueryable<T> Sort<T>(IQueryable<T> queryable,IEnumerable<Sort> sort)
        {
            if (sort.Any())
            {
                string ordering = string.Join(",", sort.Select(x => $"{x.Field} {x.Dir}"));
        ...
        public static string Transform(Filter filter,IList<Filter> filters)
        {
            ...
            if (!string.IsNullOrEmpty(filter.Value))
            {
                if (filter.Operator == "doesnotcontain")
                    where.Append($"(!np{filter.Field}).{comprasion}(@{index}))");
                else if (comprasion == "StartsWith" || comprasion == "EndsWith" || comprasion == "Contains")
                    where.Append($"(np({filter.Field}).{comprasion}(@{index}))");
                else
                    where.Append($"np({filter.Field}) {comprasion} @{index}");
            }
            else if (filter.Operator == "isnull" || filter.Operator == "isnotnull")
                where.Append($"np({filter.Field}) {comprasion}");
